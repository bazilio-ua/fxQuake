...
void PF_precache_sound (void)
{
	...
	for (i=0 ; i < ((sv.protocol == PROTOCOL_NETQUAKE) ? 256 : MAX_SOUNDS) ; i++)
	{
	...
	PR_RunError ("PF_precache_sound: overflow, max = %d", (sv.protocol == PROTOCOL_NETQUAKE) ? 256 : MAX_SOUNDS);
}
...
void PF_precache_model (void)
{
	...
	for (i=0 ; i < ((sv.protocol == PROTOCOL_NETQUAKE) ? 256 : MAX_MODELS) ; i++)
	{
	...
	PR_RunError ("PF_precache_model: overflow, max = %d", (sv.protocol == PROTOCOL_NETQUAKE) ? 256 : MAX_MODELS);
}
...
void PF_aim (void)
{
	...
	(void) speed; // fix warning 'variable set but not used'
...
{
	PF_Fixme,
	PF_makevectors,		// void(entity e) makevectors = #1; // sets v_forward, etc globals
	PF_setorigin,		// void(entity e, vector o) setorigin = #2;
	PF_setmodel,		// void(entity e, string m) setmodel = #3; // set movetype and solid first
	PF_setsize,			// void(entity e, vector min, vector max) setsize = #4;
	PF_Fixme,			// void(entity e, vector min, vector max) setabssize = #5; // #5 was removed
	PF_break,			// void() break = #6;
	PF_random,			// float() random = #7; // returns 0 - 1
	...
	PF_checkclient,		// entity() clientlist = #17; // returns a client to look for
	...
	PF_coredump,		// void() coredump = #28; // prints all edicts
	PF_traceon,			// void() traceon = #29; // turns statment trace on
	PF_traceoff,		// void() traceoff = #30;
	PF_eprint,			// void(entity e) eprint = #31; // debug prints an entire edict
	PF_walkmove, 		// float(float yaw, float dist) walkmove = #32; // returns TRUE or FALSE
	PF_Fixme, 			// #33 was removed
	PF_droptofloor,		// float() droptofloor = #34; // TRUE if landed on floor
	PF_lightstyle,		// void(float style, string value) lightstyle = #35;
	PF_rint,			// float(float v) rint = #36; // round to nearest int
	PF_floor,			// float(float v) floor = #37; // largest integer <= v
	PF_ceil,			// float(float v) ceil = #38; // smallest integer >= v
	PF_Fixme,			// #39 was removed
	PF_checkbottom,		// float(entity e) checkbottom = #40; // true if self is on ground
	PF_pointcontents,	// float(vector v) pointcontents = #41; // returns a CONTENT_*
	PF_Fixme,			// #42 was removed
	PF_fabs,			// float(float f) fabs = #43;
	PF_aim,				// vector(entity e, float speed) aim = #44; // returns the shooting vector
	PF_cvar,			// float(string s) cvar = #45; // return cvar.value
	PF_localcmd,		// void(string s) localcmd = #46; // put string into local que
	PF_nextent,			// entity(entity e) nextent = #47; // for looping through all ents
	PF_particle,		// void(vector o, vector d, float color, float count) particle = #48; // start a particle effect
	PF_changeyaw,		// void() ChangeYaw = #49; // turn towards self.ideal_yaw at self.yaw_speed
	PF_Fixme,			// #50 was removed
	PF_vectoangles,		// vector(vector v) vectoangles = #51;
	
	PF_WriteByte,		// void(float to, float f) WriteByte = #52; // direct client message generation
	PF_WriteChar,		// void(float to, float f) WriteChar = #53; // direct client message generation
	PF_WriteShort,		// void(float to, float f) WriteShort = #54; // direct client message generation
	PF_WriteLong,		// void(float to, float f) WriteLong = #55; // direct client message generation
	PF_WriteCoord,		// void(float to, float f) WriteCoord = #56; // direct client message generation
	PF_WriteAngle,		// void(float to, float f) WriteAngle = #57; // direct client message generation
	...
	PF_precache_file,	// string(string s) precache_file = #68; // no effect except for -copy
	PF_makestatic,		// void(entity e) makestatic = #69;
	
	PF_changelevel,		// void(string s) changelevel = #70;
	PF_Fixme,			// #71 was removed
	
	PF_cvar_set,		// void(string var, string val) cvar_set = #72; // sets cvar.value
	PF_centerprint,		// void(entity client, string s) centerprint = #73; // sprint, but in middle
	
	PF_ambientsound,	// void(vector pos, string samp, float vol, float atten) ambientsound = #74;
	
	PF_precache_model,	// string(string s) precache_model2 = #75; // registered version only
	PF_precache_sound,	// string(string s) precache_sound2 = #76; // registered version only (precache_sound2 is different only for qcc)
	PF_precache_file,	// string(string s) precache_file2 = #77; // registered version only
	
	PF_setspawnparms	// void(entity e) setspawnparms = #78; // set parm1... to the values at level start for coop respawn
};
