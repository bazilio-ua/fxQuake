...
//	if (R_CullBox (node->minmaxs, node->minmaxs+3, 15))
//		return;
    ...
    if (clipflags)
	{
		for (i=0 ; i<4 ; i++)
		{
			...
			if ((side = BoxOnPlaneSide(node->minmaxs, node->minmaxs+3, &frustum[i])) == BOX_OUTSIDE_PLANE) 
                return;
