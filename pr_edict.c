...
qboolean	pr_fullbright_supported;
...
edict_t *ED_Alloc (void)
{
	...
	if (i == MAX_EDICTS)
		Host_Error ("ED_Alloc: no free edicts, max = %d", MAX_EDICTS);
...
void ED_Write (FILE *f, edict_t *ed)
{
	...
	// save entity alpha manually when progs.dat doesn't know about alpha
	if (!pr_alpha_supported && ed->alpha != ENTALPHA_DEFAULT)
		fprintf (f,"\"alpha\" \"%f\"\n", ENTALPHA_TOSAVE(ed->alpha));
	// the same for fullbright
	if (!pr_fullbright_supported && ed->fullbright != 0)
		fprintf (f,"\"fullbright\" \"%f\"\n", ed->fullbright);
...
char *ED_ParseEdict (char *data, edict_t *ent)
{
	...
	while (1)
	{
		...
		// hack to support .alpha even when progs.dat doesn't know about it
		if (!strcmp(keyname, "alpha"))
			ent->alpha = ENTALPHA_ENCODE(atof(com_token));
		if (!strcmp(keyname, "fullbright"))
			ent->fullbright = atof(com_token);
